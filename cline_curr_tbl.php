...
if (!$atable_name) {
        $command_line_result_arr[] = AfwUtils::hzm_format_command_line("error", "no table specified");
        $nb_errors++;
        $command_finished = true;
        return;
} elseif (!$module_code) {
        $command_line_result_arr[] = AfwUtils::hzm_format_command_line("error", "no module specified");
        $nb_errors++;
        $command_finished = true;
        return;
} else {
        ...
        if (!$objModule) {
                ...
                if ($objAtable and (!$objAtable->isEmpty())) {
                        ...
                        if ($currtbl_code != $atable_name) {
                                ...
                                $command_line_result_arr[] = AfwUtils::hzm_format_command_line("info", "current table changed to $atable_class id of table is $idTab, id of module is $idMod, it contain $nbFields field(s)");

                                $command_line_result_arr[] = AfwUtils::hzm_format_command_line("success", $atable_translated . " : " . $objAtable->getDisplay($lang) . "\n" . $linkBtn, $lang);
                                list($isOk, $dataErr) = $objAtable->isOk(true, true);
                                if (!$isOk) {
                                        $command_line_result_arr[] = AfwUtils::hzm_format_command_line("warning", "There are warnings : " . var_export($dataErr, true), $lang);
                                }
                                $currfld = '';
                        } else {
                                $command_line_result_arr[] = AfwUtils::hzm_format_command_line("info", "current table is already $atable_name : " . $objAtable->getDisplay($lang) . "\n" . $linkBtn, $lang);
                        }
                } else {
                        $command_line_result_arr[] = AfwUtils::hzm_format_command_line("error", "table $atable_name not found in module $module_code (id=$idMod) objAtable=" . var_export($objAtable, true));
                        $nb_errors++;
                        $command_finished = true;
                        return;
                }
