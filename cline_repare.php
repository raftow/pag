...
$repareByCodeArr["afield"] = true;


$what_to_repare = $command_line_words[1];

list($object_table, $object_module) = ClineUtils::parse_table_and_module($what_to_repare);
// die("log rafik : ($object_table, $object_module) = ClineUtils::parse_table_and_module($what_to_repare);");
if(!$object_table)
{
    if ($currmod) 
    {
...
if(!$object_code)
{
    $command_line_result_arr[] = hzm_format_command_line("error", "repare command need the thing to repare !! try to see {help repare}");
...
if (file_exists("$module_path/$object_table.php")) {
    ...
    if ($object_code) {
        if ($repareByCodeArr[$object_table]) {            
            ...
            $command_line_result_arr[] = hzm_format_command_line("error", "repare $object_class by code still not implemented in Momken framework comand line");
            $nb_errors++;$command_finished = true;return;
        }
    }
    /*
...
{
    $command_line_result_arr[] = hzm_format_command_line("error", "Error 0005 when reparing. Please check that the file '$object_table.php' file exists in module path '$module_path'");
    $nb_errors++;$command_finished = true;return;
}
// hzm_stop_immediate_output();
if(!$objToShow)
{
    $command_line_result_arr[] = hzm_format_command_line("error", "Error 0001 repare $object_class by code failed with message $message");
    $nb_errors++;$command_finished = true;return;
}
else
{
