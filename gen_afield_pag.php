...
{ 
  ...
  //"***".mb_detect_encoding($s)."*** $s"; 
}

//$imposed_charset = "AR8ISO8859P6"; 
...
{
       $data_all_tab = recup_data("select id as ATABLE_ID, ATABLE_NAME, TITRE as TITLE from ATABLE where id_module = $pm_id and avail = 'Y'"); 
...
if($table_name and $fields)
{
      ...
      $atable_row = recup_row("select id from ATABLE where ATABLE_NAME = '$table_name'");
      $atable_id = $atable_row["id"];
      ...
      $data_existing_afields_pag = recup_data("select concat(ATABLE_ID,'.',FIELD_NAME) as AFIELD_CODE , id as AFIELD_ID, FIELD_NAME, ANSWER_TABLE_ID, titre as TITLE from AFIELD where avail = 'Y'");
      $index_existing_afields_pag = get_tableau_byid($data_existing_afields_pag,"AFIELD_CODE");
          ...
          where tab.table_name = '$reel_table_name'
           ...
           foreach($data_afield as $i => $row_afield)
           {
                        ...
                        if(!field_is_to_ignore($field_name)) 
                        {
                                   ...
                                   $title_after = "";
                                   $answer_table_id = "";
                                   $title = $index_existing_afields[$afield_code]["TITLE"];
                                   $title = cp1251_to_utf8($title);
                                   if(!$title) $title = cp1251_to_utf8($field_title);
                                   $answer_table_id = $index_existing_afields[$afield_code]["ANSWER_TABLE_ID"];
                                   $title_after = $index_existing_afields[$afield_code]["TITLE_AFTER"];
                            ...
                            //           $afield_type_id = 12;  // 12	enum	قائمة قصيرة	Short list
                                   ...
                                   if($answer_table_name and (!$answer_table_id))
                                   {
                                   ...
                                   $version = 0;
                                   if((!$title) or (!$answer_table_id) or ($answer_table_id=="???"))
                                   { 
                                           ...
                                           if(!$title) $title = cp1251_to_utf8($similar_field_row["TITLE"]);
                                   ...
                                   if(!$index_existing_afields_pag[$afield_code])
                                   {    
                                          $sql_generated .= "\n insert into pag.afield (atable_id,field_name,reel,additional,titre_short,title_after,mandatory,need_validation,entry_type_id,afield_type_id,titre,avail,answer_table_id,version) values ";
                                          $sql_generated .= "($atable_id,'$field_name','$reel','$additional','$title','$title_after','$mandatory','$need_validation',$entry_type_id,$afield_type_id,'$descr','$avail',$answer_table_id,$version);";
                                   }
                                   /*
                                   else
                                   {
...
?>
<center>
<form id="genSQL" name="genSQL" action="gen_afield_pag.php" method="POST">
