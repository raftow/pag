...
if (isset($_GET['m']))
        $module = $_GET['m'];
if (!$module)
        die('please set module ex icode.php?m=adm');
...
foreach ($struc_lines_arr as $struc_line) {
        ...
        if (($field_name != 'id') and
                ...
                if (!$php_c)
                        die("no template for fn=$field_name, ot=$oracle_type => F_TEMPLATE[$afwTypeFull] not found after list($afwType, $asize, $tsize, $maxlength, $utf8, $fk_table, $pct, $lang) = oracleTypeToAfwType($field_name, $oracle_type) 
