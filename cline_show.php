...
if (count($command_line_words) > 3) {
    $command_line_result_arr[] = AfwUtils::hzm_format_command_line("error", "command line $command_code does'nt need all these params, see help.");
    $nb_errors++;
...
if (!$command_mode) {
    $command_line_result_arr[] = AfwUtils::hzm_format_command_line("error", "can't complete display command, template not found");
    $nb_errors++;
    $command_finished = true;
    return;
}
...
if (file_exists("$file_dir_name/../$object_module/models/$object_table.php")) {
    ...
    $command_line_result_arr[] = AfwUtils::hzm_format_command_line("error", "3.please check that class $object_table file exists in module '$object_module'");
    $nb_errors++;
    $command_finished = true;
    return;
}
...
if ($objToShow and (!$objToShow->isEmpty())) {
    ...
    $command_line_result_arr[] = AfwUtils::hzm_format_command_line("success", $myDisplayAndLink, $lang, false, true);
    $odd_oven = "odd";
    ...
    $command_line_result_arr[] = AfwUtils::hzm_format_command_line("error", "object $object_class [$object_codeOrId] not found");
    $nb_errors++;
    $command_finished = true;
    return;
}
