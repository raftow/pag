...
if (!$objModule) {
        $command_line_result_arr[] = UfwUtils::hzm_format_command_line("error", "current module not set (use curr command)");
        $nb_errors++;
} else {
        ...
        if ($objAtable and (!$objAtable->isEmpty())) {
                ...
                $command_line_result_arr[] = UfwUtils::hzm_format_command_line("error", "table $atable_name not found in module $currmod");
                $nb_errors++;
        }
}
$command_done = true;
