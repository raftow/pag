...
class Atable extends AFWObject
{
    ...
    public static function loadByCodes($object_code_arr, $create_if_not_exists_with_name = '', $lang = 'ar', $rename_if_exists = false)
    {
        if (count($object_code_arr) != 2)
            throw new AfwRuntimeException('Atable::loadByCodes : 2 params needed (module_code, table_name) : object_code_arr=' . var_export($object_code_arr, true));
        ...
        if ((!$objModule) or $objModule->isEmpty())
            throw new AfwRuntimeException("Atable::loadByCodes : can't find module by module_code = $module_code");
    ...
    public static function generateTablePrevileges($module, $atable_name, $goal_id, $action = '+t', $framework_mode = 'qsearch', $jobrole_id = null)
    {
        ...
        if ($pure_dev_mode) {
            ...
            if (is_integer($module)) {
                ...
                if (!$objModule)
                    throw new AfwRuntimeException("generateTablePrevileges : Module `$module` not found");
            ...
            if ($objAtable and (!$objAtable->isEmpty())) {
                ...
                throw new AfwRuntimeException("generateTablePrevileges : Table `$atable_name` not found in module `$module`");
        } else
            return 'In mode test or prod no need to generate table previleges';
    }
    ...
    public function manageMeInMenuOf($goal_id, $jobrole_id, $framework_mode, $action = '+t')
    {
        ...
        if ($error)
            throw new AfwRuntimeException("generateTablePrevileges::objAtable::genereUserBFs : error : $error");
        AfwAutoLoader::addModule('bau');
        $goalObj = Goal::loadById($goal_id);
        if (!$goalObj)
            throw new AfwRuntimeException("generateTablePrevileges : goal `$goal_id` not found");
        list($error, $info) = $goalObj->manageTable($this->id, $jobrole_id, $action, $framework_mode);
        if ($error)
            throw new AfwRuntimeException("generateTablePrevileges : error : $error");
    ...
    {
        if ($code == 'djs')
            return $this->getDisplayJobroles();
        if ($code == 'em')
            return $this->getEntityManagerJobroles();
        if ($code == 'bm')
            return $this->getBusinessManagerJobroles();
        if ($code == 'sm')
            return $this->getSystemManagerJobroles();
        if ($code == 'mm')
            return $this->getMonitoringManagerJobroles();
    ...
    protected function getOtherLinksArray($mode, $genereLog = false, $step = 'all')
    {
        ...
        if ($nf)
            $next_atable_id = $nf->getId();
        else
            $next_atable = 0;
        if ($pf)
            $previous_atable_id = $pf->getId();
        else
            $previous_atable_id = 0;
    ...
    {
        if (isset($this->debugg_is_detail_table_for_others))
    ...
    public function countDetailTables($entity_relation_type_id = 1, $atable_id_list_arr = null)
    {
        ...
        $af->select('reel', 'Y');
        $af->select('afield_type_id', 5);
        if ($entity_relation_type_id)
            $af->select('entity_relation_type_id', $entity_relation_type_id);
    ...
    public function getFKPartOfMeTablesRelationFields($option = '', $option_doesnt_exist = false)
    {
        ...
        if ($option) {
            if ($option_doesnt_exist)
                $af->where("foption_mfk is null or foption_mfk not like '%,$option,%'");
            else
                $af->where("foption_mfk like '%,$option,%'");
        }
    ...
    public function getFKNotPartOfMeTablesRelationFields($option = '', $option_doesnt_exist = false)
    {
        ...
        if ($option) {
            if ($option_doesnt_exist)
                $af->where("foption_mfk not like '%,$option,%'");
            else
                $af->where("foption_mfk like '%,$option,%'");
        }
    ...
    public function getFileAndClass()
    {
        ...
        if (!$this->getVal('id_module'))
            throw new AfwRuntimeException('can not getFileAndClass, id_module is not defined for this table ');
        $module = $this->myModuleCode();
    ...
    public function getAFieldAfter($afield, $from_all_table = true)
    {
        ...
        if (!$from_all_table)
            $scenario_item_id = $afield->getVal('scenario_item_id');
        if (!$afield_order)
            $afield_order = 0;
        ...
        if (!$from_all_table)
            $af->select('scenario_item_id', $scenario_item_id);
    ...
    public function getMyNameFieldList()
    {
        ...
        foreach ($af_list as $af_id => $af_item)
            $af_list_arr[] = $af_item->getVal('field_name');
    ...
    public function getAFieldList($virtual = false, $reel = true, $additional = false, $original = true, $scenario_item_id = 0)
    {
        ...
        if (!is_array($af_list_arr)) {
            ...
            if (!$virtual and $reel)
                $af->select('reel', 'Y');
            if (!$original and $additional)
                $af->select('additional', 'Y');
            if ($original and !$additional)
                $af->select('additional', 'N');
            if ($scenario_item_id)
                $af->select('scenario_item_id', $scenario_item_id);
            ...
            foreach ($af_list as $af_id => $af_item)
                $af_list_arr[$af_item->getVal('field_name')] = $af_item;
    ...
    public function getFormuleResult($attribute, $what = 'value')
    {
        ...
        switch ($attribute) {
            ...
            case 'advise':
                ...
                if ($this->isActive() and (!$this->_isEnum()))  //  1432 = BF_IF admin/qedit of atable
                {
                    if (($objme) and ($objme->iCanDoBF(1432))) {
                        ...
                        if ($advise_html)
                            $advise_html = "<div class='error'>$advise_html</div>";
                    } else {
            ...
            case 'concernedGoalList':
                AfwAutoLoader::addModule('bau');
                if ($this->getId() > 0)
                    $goalList = GoalConcern::getJobRoleGoalListUsingTable(0, $this->getId());
                else
                    $goalList = array();
                return $goalList;
                break;
    ...
    public function myModuleCode()
    {
        ...
        if (!self::$moduleCodeOfId[$this_id_module]) {
            $moduleObj = $this->getModule();
            if (!$moduleObj)
                self::$moduleCodeOfId[$this_id_module] = "module-$this_id_module-not-found";
            else
                self::$moduleCodeOfId[$this_id_module] = $moduleObj->getVal('module_code');
            if (!self::$moduleCodeOfId[$this_id_module])
                self::$moduleCodeOfId[$this_id_module] = "module-$this_id_module-has-no-code";
        }

        return self::$moduleCodeOfId[$this_id_module];
    }
    ...
    public static function getJobRoleGoalCountUsingTable($jobrole_id, $atable_id)
    {
        $obj = new GoalConcern();
        if (!$atable_id)
            $obj->_error('getJobRoleGoalCountUsingTable : atable_id is mandatory field');
    ...
    public function getNodeDisplay($lang = 'ar')
    {
        ...
        if ($nbG == 0)
            $fn = trim($fn . " <span class='badge badge-secondary error'>new</span>");
        else
            $fn = trim($fn . " <span class='badge badge-secondary'>$nbG</span>");
    ...
    public function getDisplayField($lang = 'ar')
    {
        ...
        if ($display_field == 'id')
            $display_field = '';
        if (!$display_field)
            $display_field = $this->getVal('atable_name') . '_name';

        if (!$this->getAFieldByFieldName($display_field))
            $display_field .= "_$lang";
        else
            return $display_field;
        if (!$this->getAFieldByFieldName($display_field))
            $display_field = "titre_short_$lang";
        else
            return $display_field;
        if (!$this->getAFieldByFieldName($display_field))
            $display_field = "name_$lang";
        else
            return $display_field;
        if (!$this->getAFieldByFieldName($display_field)) {
    ...
    public function generatePhpClass($dbstruct_only = false, $dbstruct_outside = false)
    {
        ...
        foreach ($af_reel_list as $af_reel_id => $af_reel_obj) {
            ...
            if (!$af_reel_obj->is('reel'))
                $tabulation_additionelle = "\t";
            else
                $tabulation_additionelle = '';
...
";
            ...
            foreach ($indx_afield_list as $indx_afield_id => $indx_afield_obj) {
                if ($indx_afield_obj->_isMandatory())
                    $mandatory_indx_cols[$indx_afield_obj->getVal('field_name')] = true;
            }
        }
        ...
        foreach ($omr_field_arr as $omr_field_id => $omr_field_obj) {
            ...
            if ((strlen($omr_ans_tab_abrev) > 7) and (strlen($omr_short_name) > 20))
                $omr_short_abrev = $omr_ans_tab_abrev;
            else
                $omr_short_abrev = $omr_short_name;

            if (strlen($omr_short_abrev) > 14)
                $vfield_shortname = '';
            else
                $vfield_shortname = $omr_short_abrev . 's';
            ...
            if ($vfield_shortname)
                $virtual_field->set('shortname', $vfield_shortname);
...
";
        ...
        foreach ($name_cols as $i => $name_col) {
            $name_with_language = '';
            if (AfwStringHelper::stringStartsWith($name_col, '_ar'))
                $name_with_language = 'ar';
            if (AfwStringHelper::stringStartsWith($name_col, '_fr'))
                $name_with_language = 'fr';
            if (AfwStringHelper::stringStartsWith($name_col, '_en'))
                $name_with_language = 'en';
...
";
    ...
    public function generateSQLStructure()
    {
        ...
        if ((!$dbms) or (!$dbsystem) or (!$dbengine))
            return "Please define db system and engine of module [$module] for the table [$this]";
        ...
        foreach ($af_reel_list as $af_reel_id => $af_reel_obj) {
            ...
            if ($fields_naming_uc)
                $af_field_name = strtoupper($af_field_name);
        ...
        foreach ($af_fk_list as $af_fk_id => $af_fk_obj) {
            ...
            if ($fk_table->getVal('id_module') == $this->getVal('id_module')) {
                ...
                if ($fields_naming_uc)
                    $fk_field_name = strtoupper($fk_field_name);
                if ($fields_naming_uc)
                    $fk_table_pk = strtoupper($fk_table_pk);

                $fk_table_name = $fk_table->valAtable_name();
                if ($tables_naming_uc)
                    $fk_table_name = strtoupper($fk_table_name);
...
";
    ...
    public function beforeMaj($id, $fields_updated)
    {
        ...
        if ($this->isLookup()) {
            ...
            if ($module)
                $module->updateLookupGoal();

            if (!$this->getVal('jobrole_id')) {
                $jr_lookup = null;
                if ($module)
                    list($jr_lookup, $goal_lookup) = $module->getLookupJobResp(true, true);

                if ($jr_lookup)
                    $this->set('jobrole_id', $jr_lookup->getId());
            }
        }
    ...
    public function beforeDelete($id, $id_replace)
    {
        ...
        if ($id) {
            if ($id_replace == 0) {
                ...
                // if there's no record that block the delete operation perform the delete of the other records linked with me and deletable
                if (!$simul)
                    $obj->deleteWhere("curr_class_atable_id = '$id' and avail='N'");
    ...
    public function createTranslationFieldsTo($lang = 'en', $in_lang_name_ar = 'بالانجليزية', $lang_name_en = '')
    {
        ...
        if ((!$this->ignore_initial_translation_fields) and ($field_exists_count == 0)) {
            if ($this->_isEnum() or $this->_isLookup()) {
                ...
                $my_atable_name = $this->getVal('atable_name');
                if (!$display_field)
                    $display_field = $my_atable_name . '_name';
                $af_new = new Afield();
    ...
    public function createDefaultFields($lang = 'ar')
    {
        ...
        if ($fieldcount < 3) {
            if ((!AfwSession::hasOption('CLINE-NO-DEFAULT-FIELDS')) and (!$this->NO_DEFAULT_FIELDS)) {
                ...
                if (!$table_titre_u_en)
                    $table_titre_u_en = AfwStringHelper::toEnglishText($this_table_name);
                $atable_id = $this->getId();
                ...
                return array('لا يسمح بانشاء الحقول المبدئية لمثل هذه الجداول', '');
        }
    ...
    public function createUpdateMySteps($lang = 'ar')
    {
        ...
        foreach ($sci_list as $sci_item) {
            ...
            if ($cum_steps[$step_num]['stepname_ar'] != $stepcode)
                $sci_item->set('step_name_ar', $cum_steps[$step_num]['stepname_ar']);
            if (($cum_steps[$step_num]['stepname_en'] != $stepcode) or (!$sci_item->valstep_name_en()))
                $sci_item->set('step_name_en', $cum_steps[$step_num]['stepname_en']);
    ...
    {
        if ($this->table_category)
            return $this->table_category;

        if (!$this->is('real_table')) {
            if ($this->findInMfk('tboption_mfk', self::$TBOPTION_OBJ_IS_VIEW, false))
                $this->table_category = 'view';
            else
                $this->table_category = 'vobj';
        } elseif ($this->_isEnum())
            $this->table_category = 'enum';
        elseif ($this->_isLookup()) {
            // die("is lookup");
            if ($this->hasManyData())
                $this->table_category = 'blookup';
            else
                $this->table_category = 'lookup';
        ...
        } elseif ($this->_isEntity())
            $this->table_category = 'entity';
        else
            $this->table_category = 'relation';

        return $this->table_category;
    }

    public function minLevelForAll($lang = 'ar')
    {
        $bfunctionList = $this->get('bfunctionList');
        $arrLeveled = [];
        ...
        $countLeveled = count($arrLeveled);

        $message = "successfully done : $countLeveled BF(s) has been leveled : <br>\n" . implode("<br>\n", $arrLeveled);
    ...
    public function genereUserBFs($lang = 'ar', $hierarchy_level_settings = [])
    {
        UfwQueryAnalyzer::startProcessLourdMode();

        $framework_id = AfwSession::config('framework_id', 1);
        list($err0, $inf0) = $this->disableMyAutoGeneratedBFs($lang);
        $bf_arr = $this->createFrameWorkScreens($framework_id, true, true, $hierarchy_level_settings);
    ...
    public function createModeScreen($framework_mode, $framework_id = 0, $resetUS = false, $force = false)
    {
        ...
        if (!$system_id)
            throw new AfwRuntimeException("failed $this --> createFrameWorkScreens($framework_id,$resetAll) system is not defined for this table (id=$this_id)");
        ...
        if ($framework_mode_item) {
            ...
            if (!$bf_type_code)
                $bf_type_code = $framework_mode_item['bf_type']['all'];
            if ($bf_type_code)
                $bf_type = Bfunction::${"BFUNCTION_TYPE_$bf_type_code"};
            else
                $bf_type = '';
            if (!$bf_type) {
            ...
            if ($framework_mode_item['categories'][$cat]) {
                ...
                if (is_object($bf) and ($bf->getId() > 0)) {
                    ...
                    throw new AfwRuntimeException("failed Bfunction::getOrCreate-BF($system_id, $file_name, $id_module, $this_id, $bf_spec, $titre, $titre_en, $titre, $titre_en, $direct_access, $public, $bf_type, $bf_code) : " . var_export($bf, true));
                }
            } else {
    ...
    public function createFrameWorkScreens($framework_id = 0, $resetAll = true, $resetUS = true, $hierarchy_level_settings = [])
    {
        ...
        foreach ($framework_mode_list as $framework_mode => $framework_mode_item) {
            ...
            if ($framework_mode_applicable_to_category and (($framework_mode != 'stats') or ($this->hasOption(self::$TBOPTION_GEN_STATS)))) {
                ...
                $hierarchy_level = $hierarchy_level_settings[$atable_name . "-" . $framework_mode];
                if (!$hierarchy_level) $hierarchy_level = $hierarchy_level_settings[$atable_name];
                if (!$hierarchy_level) $hierarchy_level = 999;
                /** @var Bfunction $bf */
                list($bf, $bf_new) = Bfunction::getOrCreateBF(
                    $system_id,
                    $file_name,
    ...
    protected function getPublicMethods()
    {
        ...
        if ($this->_isEntity()) {
            ...
            $pbms['5a1hJ5'] = array(
                'METHOD' => 'disableMyAutoGeneratedBFs',
                'COLOR' => $color,
                'LABEL_AR' => $title_ar,
                'ADMIN-ONLY' => true,
                'MODE' => array('mode_bfunctionList' => true),
            );
    ...
    public function getLookupValuesFromDB($lang = 'ar')
    {
        ...
        if (true) {
            ...
            if ($my_module)
                AfwAutoLoader::addModule($my_module);
            ...
            foreach ($obj_list as $obj_item) {
                ...
                if (!$lookup_code)
                    $lookup_code = $obj_item->getVal('lookup_code');
                if (!$lookup_code)
                    $lookup_code = get_class($obj_item) . '-' . $obj_item->id;
                if ($lookup_code) {
    ...
    protected function getSpecificDataErrors($lang = 'ar', $show_val = true, $step = 'all', $erroned_attribute = null, $stop_on_first_error = false, $start_step = null, $end_step = null)
    {
        ...
        if ($this->stepContainAttribute($step, 'id_sub_module')) {
            if ($this->getVal('id_sub_module') == $this->getVal('id_module'))
                $sp_errors['id_sub_module'] = 'Module and Sub-module should be different in new version of hazm framework';
        }
        $system = $this->het('system_id');
        $application = $this->het('id_module');
        if ($this->stepContainAttribute($step, 'system_id')) {
            if (!$system)
                $sp_errors['system_id'] = 'System not defined !!';
        }
        if ($this->stepContainAttribute($step, 'id_module')) {
            if (!$application)
                $sp_errors['id_module'] = 'Application not defined !!';
            if ($application) {
                if ($system) {
                    if ($application->getVal('id_system') != $system->getId())
                        $sp_errors['id_module'] = "Application should belong to system $system";
                }
            }
        }
        ...
        if ($this->stepContainAttribute($step, 'id_sub_module') and $this->getVal('id_sub_module')) {
            ...
            if (!$sub_module)
                $sp_errors['id_sub_module'] = 'Sub-module is mandatory';
            else {
                if ($system) {
                    if ($sub_module->getVal('id_system') != $system->getId())
                        $sp_errors['id_sub_module'] = "Sub-module should be from system $system";
                }

                if ($application) {
                    $parentAppl = $sub_module->getParentApplication();
                    if ((!$parentAppl) or ($parentAppl->getId() != $application->getId()))
                        $sp_errors['id_sub_module'] = "Sub-module should be from application $application";
                }
            }
        }
        ...
        if ($this->stepContainAttribute($step, 'indexFieldList')) {
            if ($this->attributeIsApplicable('indexFieldList')) {
                ...
                if (!$this->hasOption(self::$TBOPTION_NEVER_GENERATED)) {
                    ...
                    if (!$indexCount)
                        $sp_errors['indexFieldList'] = "for this category of table : $categ, index fields are mandatory to define or option 'never generated' should be checked";
                }
        ...
        if ($this->stepContainAttribute($step, 'scis') and $this->attributeIsApplicable('scis')) {
            ...
            if (($afCount > 17) or (($afCount > 10) and (!$this->hasOption(self::$TBOPTION_OPEN_ROLES_ON_FULL_SCREEN)))) {
                $need_tabs = true;
                if (!$scis_count)
                    $sp_errors['scis'] = "For this category of tables : $categ, when field count exceed 10 we need to broke screen into tabs or define only one screen option";
            }
            ...
            if ($afCount > $max_count) {
                ...
                $scis_count = count($scis);
                if (!$scis_count)
                    $sp_errors['scis'] = "For this category of tables : $categ, when field count exceed $max_count we need to broke screen into tabs";
            }

            if (($scis_count) and ($need_tabs)) {
                $afNonAssignedToStepCount = $this->getAFieldNonAssignedToStepCount();
                if ($afNonAssignedToStepCount > 0)
                    $sp_errors['scis'] = "Still $afNonAssignedToStepCount fields non assigned to their step";
            }
        }
    ...
    public function getRAMObjectData()
    {
        ...
        if (!$code)
            throw new AfwRuntimeException('this table is without table name');
    ...
    {
        if ($fgroup == 'scis')
            return array('name' => $fgroup, 'css' => 'pct_100');
    ...
    public function myShortNameToAttributeName($attribute)
    {
        ...
        if ($attribute == 'detail')
            return 'is_detail';
        if ($attribute == 'pluraltitle')
            return 'titre_short';
        if ($attribute == 'pluraltitleshort')
            return 'titre_short_s';
        if ($attribute == 'singletitle')
            return 'titre_u';
        if ($attribute == 'singletitleshort')
            return 'titre_u_s';
        if ($attribute == 'description')
            return 'titre';
        if ($attribute == 'submodule')
            return 'id_sub_module';
        if ($attribute == 'icon')
            return 'vh';
        if ($attribute == 'pluraltitle_en')
            return 'titre_short_en';
        if ($attribute == 'singletitle_en')
            return 'titre_u_en';
        if ($attribute == 'description_en')
            return 'titre_en';
        if ($attribute == 'original_fields')
            return 'origFieldList';
        if ($attribute == 'audit_fields')
            return 'auditFieldList';
        if ($attribute == 'index_fields')
            return 'indexFieldList';
        if ($attribute == 'name_fields')
            return 'nameFieldList';
        if ($attribute == 'virtual_fields')
            return 'virtFieldList';
        if ($attribute == 'additional_fields')
            return 'addiFieldList';
        if ($attribute == 'emjob')
            return 'jobrole_id';
        if ($attribute == 'maxper')
            return 'max_period';
        if ($attribute == 'expper')
            return 'exp_period';
        if ($attribute == 'minrows')
            return 'min_u_records';
        if ($attribute == 'exprows')
            return 'exp_u_records';
        return $attribute;
    }
    ...
    {
        if (count($object_code_arr) != 2)
            throw new AfwRuntimeException('Atable::levelByCodes : 2 params are needed module and table, given : ' . var_export($object_code_arr, true));
        $table_name = $object_code_arr[0];
        $module_code = $object_code_arr[1];
        if (!$module_code or !$table_name)
            throw new AfwRuntimeException("Atable::levelByCodes : module and table are needed, given : module=$module_code and table=$table_name");
        if (!$new_level or !$filter)
            throw new AfwRuntimeException('Atable::levelByCodes : new_level and filter of BFs params are required');
        $objModule = Module::loadByMainIndex($module_code);
        if (!$objModule or (!$objModule->id))
            throw new AfwRuntimeException("levelByCodes : module $module_code not found");
        $objModule_id = $objModule->id;

        $objLevel = $objModule->getLevelUgroup($new_level);
        if (!$objLevel)
            return [null, "unknown hierarchy level $new_level for module $module_code, use list all levels `ll` command to check"];

        $objTable = Atable::loadByMainIndex($objModule_id, $table_name);
        if (!$objTable or (!$objTable->id))
            throw new AfwRuntimeException("levelByCodes : table $table_name in module $module_code not found");

        $bfunctionList = $objTable->get('bfunctionList');
        $arrLeveled = [];

        /** @var Bfunction $bfunctionItem */
        foreach ($bfunctionList as $bfunctionItem) {
            $bf_title = $bfunctionItem->getNodeDisplay($lang);
            if (AfwStringHelper::stringContain($bf_title, $filter)) {
                $bfunctionItem->set('hierarchy_level_enum', $new_level);
                if ($bfunctionItem->commit()) {
                    $arrLeveled[] = $bf_title;
                }
            }
    ...
    public static function addByCodes(
        ...
        $module_code = $object_code_arr[1];
        if (!$module_code or !$table_name)
            throw new AfwRuntimeException("Atable::addByCodes : module and table are needed, given : module=$module_code and table=$table_name");
        if (!$object_name_en or !$object_name_ar or !$object_title_en or !$object_title_ar)
            throw new AfwRuntimeException("Atable::addByCodes(module=$module_code and table=$table_name) : names and titles are required (1=object_name_ar=$object_name_ar / 2=object_title_ar=$object_title_ar / 3=object_name_en=$object_name_en / 4=object_title_en=$object_title_en)");
        $objModule = Module::loadByMainIndex($module_code);
        if (!$objModule or (!$objModule->id))
        ...
        if (!$objTable)
            $message = "Strange Error happened because Atable::loadByMainIndex($objModule_id, $table_name) failed !!";
        else {
            ...
            if ($object_title_en)
                $objTable->set('titre_u_en', $object_title_en);
            if ($object_title_ar)
                $objTable->set('titre_u', $object_title_ar);
    ...
    public static function reverseByCodes($object_code_arr, $doReverse = true, $restriction = '')
    {
        ...
        $module_code = $object_code_arr[1];
        if (!$module_code or !$table_name)
            throw new AfwBusinessException("reverseByCodes : module and table are needed, given : module=$module_code and table=$table_name");
        $objModule = Module::loadByMainIndex($module_code);
        if (!$objModule or (!$objModule->id))
            throw new AfwBusinessException("reverseByCodes : module $module_code not found when reversing $table_name table");
        $objModule_id = $objModule->id;
        $message = '';
    ...
    public static function reverseTable($module_code, $table_name)
    {
        ...
        if ($mdl_new)
            ...
            $return .= ', module used : ';
        if ($mdl)
            $return .= $mdl->getDisplay('en') . '/id=' . $mdl->id;

        if ($tbl_new)
            $return .= ', new table created : ' . $tbl->getDisplay('en');
    ...
    public function getTableRightsMatrice($objectRole, $framework, $subModId = 0)
    {
        ...
        if ($moduleObj) {
            ...
            $matriceArr['error'] = "module not defined for role : $objectRole";
    ...
    public function saveTableRightsMatrice($objectRole, $framework, $data_posted, $subModId)
    {
        ...
        if ($moduleObj) {
            ...
            throw new AfwRuntimeException("module not defined for role : $objectRole");
    ...
    public function genereTranslation($lang)
    {
            ...
            if (!$this->isTechField($nom_col)) {
                ...
                if ($this->getId()) {
                    ...
                    if ($afld->load()) {
                        ...
                        $try_trad = "table $tabName need to be reverse engineeredm because can't find column [$nom_col]";
                }
        ...
        if ($langUC == 'En')
            $CLASS_NAMETranslator = $className . 'ArTranslator';
        else
            $CLASS_NAMETranslator = $className . 'EnTranslator';
