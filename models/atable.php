  ...
  PRIMARY KEY (`id`, `version`)
    ...
    public function getLookupValuesFromDB($lang = "ar")
    {
        ...
        if (true) {
            ...
            foreach ($obj_list as $obj_item) {
                ...
                if (!$lookup_code) $lookup_code = $obj_item->getVal("lookup_code");
    ...
    public function attributeIsApplicable($attribute)
    {
        ...
        if (($attribute == "goalConcernList1") or ($attribute == "goalConcernList3") or ($attribute == "goalConcernList5")) {
            ...
            return ($pure_dev_mode and $this->_isEntity() and
                $this->isOriginal() and
                (!$this->dataIsAutoGenerated()) and
                (!$this->dataIsFullOwnedByMaster()) and
                ($categ != "view") and
    ...
    {
        if (count($object_code_arr) != 2) throw new AfwRuntimeException("Atable::levelByCodes : 2 params are needed module and table, given : " . var_export($object_code_arr, true));        
        $table_name = $object_code_arr[0];
        $module_code = $object_code_arr[1];
        if (!$module_code or !$table_name) throw new AfwRuntimeException("Atable::levelByCodes : module and table are needed, given : module=$module_code and table=$table_name");
        if (!$new_level or !$filter) throw new AfwRuntimeException("Atable::levelByCodes : new_level and filter of BFs params are required");
        $objModule = Module::loadByMainIndex($module_code);
        if (!$objModule or (!$objModule->id)) throw new AfwRuntimeException("levelByCodes : module $module_code not found");
        $objModule_id = $objModule->id;
        
        $objLevel = $objModule->getLevelUgroup($new_level);
        if(!$objLevel) return [null, "unknown hierarchy level $new_level for module $module_code, use list all levels `ll` command to check"];
        
        $objTable = Atable::loadByMainIndex($objModule_id, $table_name); 
        if (!$objTable or (!$objTable->id)) throw new AfwRuntimeException("levelByCodes : table $table_name in module $module_code not found");
