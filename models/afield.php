	...
	public static $DATABASE		= ""; 
        ...
        public function myAnswerTableName()
        {
         ...
         public function attributeIsApplicable($attribute)
         {
              ...
              {
                  return (($this->getVal("afield_type_id")==5) and ($this->is("reel"))); 
              }
              ...
              {
                  return ($this->is("reel") and in_array($this->getVal("afield_type_id"),array(10, 11)));
              }
              
              if($attribute=="field_min_size")
              {
                  return ($this->is("reel") and $this->is("mandatory") and in_array($this->getVal("afield_type_id"),array(10, 11)));
              }
        ...
        protected function repareMeBeforeUpdate()
        {
            ...
            if($mode_pag_me) return true;
                ...
                {
                    if($this->is("mandatory",true)) $this->set("mandatory","Y");
                    if($this->is("mode_search",true)) $this->set("mode_search","Y");
                    if($this->is("mode_qsearch",true)) $this->set("mode_qsearch","Y");
                    if($this->is("mode_retrieve",false)) $this->set("mode_retrieve","N");
                    if($my_tab->hasOption($my_tab::$TBOPTION_DATA_IS_AUTO_GENERATED)) $this->set("readonly","Y");
                    
                     
                    // فيه نظر
                    //if($this->is("mode_name",true)) $this->set("mode_name","Y");
                }
                ...
                if(AfwStringHelper::stringStartsWith($field_name,"is_"))   
                {
                ...
                {
                        if(!$this->getVal("field_size")) $this->set("field_size",10);
                        if(!$this->is("mode_name",false)) $this->set("mode_name","N");
                }
		...
		if((AfwStringHelper::stringStartsWith($field_name,"_nb") or (AfwStringHelper::stringStartsWith($field_name,"_number"))))   
                ...
                {         
                      if(!trim($this->getVal("titre"))) 
                      {
                           if(($this->getVal("afield_type_id")==AfwUmsPagHelper::$afield_type_list) or ($this->getVal("afield_type_id")==AfwUmsPagHelper::$afield_type_enum))
                           {
                ...
                if($this->getVal("afield_type_id")==AfwUmsPagHelper::$afield_type_items)
                {
                            if(!$this->is("mode_retrieve",false)) $this->set("mode_retrieve","N");
                            if(!$this->is("mode_name",false)) $this->set("mode_name","N");
                            if($this->is("readonly",true)) $this->set("readonly","Y");
                            if($this->is("mandatory",true)) $this->set("mandatory","Y");
                }
                ...
                if($this->is("reel"))
                {
                            //if(!$this->is("readonly",false)) $this->set("readonly","N");
                }
                else
                {
                      if($this->is("readonly",true)) $this->set("readonly","Y");
        ...
        protected function getSpecificDataErrors($lang="ar",$show_val=true,$step="all",$erroned_attribute=null,$stop_on_first_error=false, $start_step=null, $end_step=null)
        {
              ...
              */
              ...
              if(($this->getVal("afield_type_id")==5) and ($this->is("reel")))
              {
        ...
        protected function getPublicMethods()
        {
            ...
            $pbms["xc123B"] = array("METHOD"=>"createShortcutFieldsForOneToOneObject",
                                     "COLOR"=>$color, 
                                     "LABEL_AR"=>$title_ar,
                                     "ADMIN-ONLY"=>true,
