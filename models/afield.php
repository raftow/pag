...
class Afield extends PagObject
{
        ...
        public static function loadByMainIndex($atable_id, $field_name, $create_obj_if_not_found = false, $activate_obj_if_disabled = false)
        {
                $obj = new Afield();
                if (!$atable_id)
                        throw new AfwRuntimeException('loadByMainIndex : atable_id is mandatory field');
                if (!$field_name)
                        throw new AfwRuntimeException('loadByMainIndex : field_name is mandatory field');

                $obj->select('atable_id', $atable_id);
                $obj->select('field_name', $field_name);

                if ($obj->load()) {
                        if ($activate_obj_if_disabled)
                                $obj->activate();
                        return $obj;
        ...
        public static function loadByCodes($object_code_arr, $create_if_not_exists_with_name = '', $lang = 'ar', $rename_if_exists = false)
        {
                if (count($object_code_arr) != 3)
                        throw new AfwRuntimeException('Afield::loadByCodes : 3 params needed (module_code, table_name, field_name) : object_code_arr=' . var_export($object_code_arr, true));
                ...
                if ((!$objModule) or $objModule->isEmpty())
                        throw new AfwRuntimeException("Afield::loadByCodes : can't find module by module_code = $module_code");

                $objTable = Atable::loadByMainIndex($objModule->getId(), $table_name);
                if ((!$objTable) or $objTable->isEmpty())
                        throw new AfwRuntimeException("Afield::loadByCodes : can't find table by table_name = $table_name in module $module_code");

                $obj = self::loadByMainIndex($objTable->getId(), $field_name, $create_if_not_exists_with_name);
                if (($obj->is_new) or $rename_if_exists) {
                        if ($lang == 'ar')
                                $obj->set('titre_short', $create_if_not_exists_with_name);
                        if ($lang == 'en')
                                $obj->set('titre_short_en', $create_if_not_exists_with_name);
                        $obj->commit();
                }
        ...
        public function dynamicHelpCondition($attribute)
        {
                if ($attribute == 'titre_short')
                        return ($this->getVal('titre_short') != $this->getVal('titre'));
        ...
        protected function repareMeBeforeUpdate()
        {
                ...
                if (trim($this->getVal('titre')) == trim($this->getVal('titre_en'))) {
                        if (!AfwStringHelper::is_arabic($this->getVal('titre')))
                                $this->set('titre', '');
                        else
                                $this->set('titre_en', '');
                }
                ...
                if ($this->getVal('titre_short') == $this->getVal('titre_short_en')) {
                        if (!AfwStringHelper::is_arabic($this->getVal('titre_short')))
                ...
                if (AfwStringHelper::stringEndsWith($field_name, '_fl')) {
                        ...
                        if (!$this->getVal('afield_type_id'))
                                $this->set('afield_type_id', AfwUmsPagHelper::$afield_type_list);
                ...
                if (AfwStringHelper::stringEndsWith($field_name, '_pk') or AfwStringHelper::stringEndsWith($field_name, '_ms'))  // ms as will be master of this record (the record will be its detail) (so one to many)
                {
                        ...
                        if (!$this->getVal('afield_type_id'))
                                $this->set('afield_type_id', AfwUmsPagHelper::$afield_type_list);
                        if (!$this->getVal('entity_relation_type_id'))
                                $this->set('entity_relation_type_id', self::$ENTITY_RELATION_TYPE_ONETOMANY);
                }
                ...
                if (AfwStringHelper::stringEndsWith($field_name, '_ou')) {
                        ...
                        if (!$this->getVal('afield_type_id'))
                                $this->set('afield_type_id', AfwUmsPagHelper::$afield_type_list);
                        if (!$this->getVal('entity_relation_type_id'))
                                $this->set('entity_relation_type_id', self::$ENTITY_RELATION_TYPE_ONETOONEUNIDIRECTIONAL);
                }
                ...
                if (AfwStringHelper::stringEndsWith($field_name, '_ob')) {
                        ...
                        if (!$this->getVal('afield_type_id'))
                                $this->set('afield_type_id', AfwUmsPagHelper::$afield_type_list);
                        if (!$this->getVal('entity_relation_type_id'))
                                $this->set('entity_relation_type_id', self::$ENTITY_RELATION_TYPE_ONETOONEBIDIRECTIONAL);
                }

                if (AfwStringHelper::stringEndsWith($field_name, '_enm') or AfwStringHelper::stringEndsWith($field_name, '_enum')) {
                        if (!$this->getVal('afield_type_id'))
                                $this->set('afield_type_id', AfwUmsPagHelper::$afield_type_enum);
                }
                ...
                if (($this->getVal('afield_type_id') == AfwUmsPagHelper::$afield_type_list) and
                        ...
                        if ($this->may('mode_search', true))
                                $this->set('mode_search', 'Y');
                        if ($this->may('mode_qsearch', true))
                                $this->set('mode_qsearch', 'Y');
                        if ($this->may('mode_retrieve', false))
                                $this->set('mode_retrieve', 'N');
                        if ($my_tab->hasOption($my_tab::$TBOPTION_DATA_IS_AUTO_GENERATED))
                                $this->set('readonly', 'Y');
                ...
                if (($this->getVal('afield_type_id') == AfwUmsPagHelper::$afield_type_list) or
                        ...
                        if (!$this->getVal('answer_table_id')) {
                                ...
                                if (!$anstab)
                                        throw new AfwRuntimeException("for field $field_name_orig this answer_table_id $anstab_id doesnt exist anymore ");
                        }
                ...
                if (($field_name == $my_tab_name . '_code') or
                        ...
                        if (!$this->getVal('titre_short_en'))
                                $this->set('titre_short_en', $my_tab_tu_en . ' code');
                        if (!$this->getVal('afield_type_id'))
                                $this->set('afield_type_id', 10);
                        if (!$this->getVal('field_size'))
                                $this->set('field_size', 16);
                        if (!$this->getVal('field_min_size'))
                                $this->set('field_min_size', 3);
                        if (!$this->getVal('char_group_men'))
                                $this->set('char_group_men', ',1,3,9,');
                }

                if ($field_name == 'lookup_code') {
                        if (!$this->getVal('titre_short'))
                                $this->set('titre_short', 'الرمز');
                        if (!$this->getVal('titre_short_en'))
                                $this->set('titre_short_en', 'Lookup code');
                        if ($this->may('mandatory', true))
                                $this->set('mandatory', 'Y');
                }
                ...
                if (AfwStringHelper::stringEndsWith($field_name, '_code')) {
                        if (!$this->getVal('titre_short'))
                                $this->set('titre_short', 'رمز .......');
                        if (!$this->getVal('titre_short_en'))
                                $this->set('titre_short_en', '........ code');
                        if (!$this->getVal('afield_type_id'))
                                $this->set('afield_type_id', 10);
                        if (!$this->getVal('field_size'))
                                $this->set('field_size', 16);
                ...
                if (AfwStringHelper::stringEndsWith($field_name, '_website')) {
                        if (!$this->getVal('titre_short'))
                                $this->set('titre_short', 'موقع .......');
                        if (!$this->getVal('titre_short_en'))
                                $this->set('titre_short_en', '........ web site');
                        if (!$this->getVal('afield_type_id'))
                                $this->set('afield_type_id', 10);
                        $this->set('field_size', 255);
                        $this->set('field_min_size', 16);
                        if (!$this->getVal('char_group_men'))
                                $this->set('char_group_men', ',1,6,4,3,7,9,');
                }

                if (AfwStringHelper::stringEndsWith($field_name, '_webpage')) {
                        if (!$this->getVal('titre_short'))
                                $this->set('titre_short', 'صفحة .......');
                        if (!$this->getVal('titre_short_en'))
                                $this->set('titre_short_en', '........ web page');
                        if (!$this->getVal('afield_type_id'))
                                $this->set('afield_type_id', 10);
                        $this->set('field_size', 255);
                        $this->set('field_min_size', 16);
                ...
                if (AfwStringHelper::stringEndsWith($field_name, '_link')) {
                        if (!$this->getVal('titre_short'))
                                $this->set('titre_short', 'رابط .......');
                        if (!$this->getVal('titre_short_en'))
                                $this->set('titre_short_en', '........ web link');
                        if (!$this->getVal('afield_type_id'))
                                $this->set('afield_type_id', 10);
                        $this->set('field_size', 255);
                        $this->set('field_min_size', 16);
                ...
                if (AfwStringHelper::stringEndsWith($field_name, '_location') or AfwStringHelper::stringEndsWith($field_name, '_gmap')) {
                        if (!$this->getVal('titre_short'))
                                $this->set('titre_short', 'رابط الموقع على الخرائط');
                        if (!$this->getVal('titre_short_en'))
                                $this->set('titre_short_en', 'google map location link');
                        if (!$this->getVal('afield_type_id'))
                                $this->set('afield_type_id', 10);
                        $this->set('field_size', 255);
                        $this->set('field_min_size', 16);
                ...
                if (($field_name == "${my_tab_name}_spec") or
                        ...
                        if (!$this->getVal('titre_short_en'))
                                $this->set('titre_short_en', $my_tab_tu_en . ' specification');
                        if (!$this->getVal('afield_type_id'))
                                $this->set('afield_type_id', 11);
                        if (!$this->getVal('field_size'))
                                $this->set('field_size', 255);
                        if (!$this->getVal('field_min_size'))
                                $this->set('field_min_size', 30);
                }
                ...
                if (($field_name == "${my_tab_name}_name") or
                        ...
                        if (!$this->getVal('titre_short_en'))
                                $this->set('titre_short_en', 'Arabic ' . $my_tab_tu_en . ' name');
                        if (!$this->getVal('afield_type_id'))
                                $this->set('afield_type_id', 10);
                        if (!$this->getVal('field_size'))
                                $this->set('field_size', 48);

                        if (!$this->getVal('field_min_size'))
                                $this->set('field_min_size', 5);
                        if (!$this->getVal('char_group_men'))
                                $this->set('char_group_men', ',2,7,');
                }
                ...
                if (($field_name == "${my_tab_name}_name_en") or
                        ...
                        if (!$this->getVal('titre_short_en'))
                                $this->set('titre_short_en', $my_tab_tu_en . ' name');
                        if (!$this->getVal('afield_type_id'))
                                $this->set('afield_type_id', 10);
                        if (!$this->getVal('field_size'))
                                $this->set('field_size', 48);
                ...
                if (($field_name == 'comment') or AfwStringHelper::stringEndsWith($field_name, '_comment')) {
                        if (!$this->getVal('afield_type_id'))
                                $this->set('afield_type_id', 10);
                        if (!$this->getVal('titre_short'))
                                $this->set('titre_short', 'ملاحظات');
                        if (!$this->getVal('titre_short_en'))
                                $this->set('titre_short_en', 'Arabic comments');
                        $this->set('field_size', 255);
                        if (!$this->getVal('field_min_size'))
                                $this->set('field_min_size', 10);
                        if (!$this->getVal('char_group_men'))
                                $this->set('char_group_men', ',11,');
                }
                ...
                if (($field_name == 'comment_en') or AfwStringHelper::stringEndsWith($field_name, '_comment_en')) {
                        if (!$this->getVal('afield_type_id'))
                                $this->set('afield_type_id', 10);
                        if (!$this->getVal('titre_short'))
                                $this->set('titre_short', 'ملاحظات بالانجليزي');
                        if (!$this->getVal('titre_short_en'))
                                $this->set('titre_short_en', 'comments');
                        $this->set('field_size', 255);
                        if (!$this->getVal('field_min_size'))
                                $this->set('field_min_size', 10);
                        if (!$this->getVal('char_group_men'))
                ...
                if ((AfwStringHelper::stringEndsWith($field_name, '_mobile') or (AfwStringHelper::stringEndsWith($field_name, '_phone') or (AfwStringHelper::stringEndsWith($field_name, '_telephone'))))) {
                        if (!$this->getVal('afield_type_id'))
                                $this->set('afield_type_id', 10);
                        if (!$this->getVal('field_size'))
                                $this->set('field_size', 20);
                        if (!$this->getVal('field_min_size'))
                                $this->set('field_min_size', 5);
                        if (!$this->getVal('char_group_men'))
                                $this->set('char_group_men', ',5,4,3,7,');
                }
                ...
                if (
                        ...
                        if (AfwStringHelper::stringEndsWith($field_name, '_title') or AfwStringHelper::stringEndsWith($field_name, '_title_ar')) {
                                if (!$this->getVal('titre_short'))
                                        $this->set('titre_short', 'عنوان ' . AfwStringHelper::arabicTaarif($my_tab_tu));
                                if (!$this->getVal('titre_short_en'))
                                        $this->set('titre_short_en', 'Arabic ' . $my_tab_tu_en . ' title');
                        } elseif (AfwStringHelper::stringEndsWith($field_name, '_title_fr')) {
                                if (!$this->getVal('titre_short'))
                                        $this->set('titre_short', 'عنوان ' . AfwStringHelper::arabicTaarif($my_tab_tu) . ' بالفرنسي');
                                if (!$this->getVal('titre_short_en'))
                                        $this->set('titre_short_en', 'titre du ' . $my_tab_tu_en);
                        } else {
                                if (!$this->getVal('titre_short'))
                                        $this->set('titre_short', 'عنوان ' . AfwStringHelper::arabicTaarif($my_tab_tu) . ' بالانجليزي');
                                if (!$this->getVal('titre_short_en'))
                                        $this->set('titre_short_en', 'titre du ' . $my_tab_tu_en);
                        }
                        if (!$this->getVal('afield_type_id'))
                ...
                if (($field_name == "${my_tab_name}_desc") or
                        ...
                        if (!$this->getVal('titre_short_en'))
                                $this->set('titre_short_en', 'Arabic ' . $my_tab_tu_en . ' description');
                }
                ...
                if (($field_name == "${my_tab_name}_desc_en") or
                        ...
                        if (!$this->getVal('titre_short_en'))
                                $this->set('titre_short_en', $my_tab_tu_en . ' description');
                }

                if (($field_name == "${my_tab_name}_date") or ($field_name == "${my_tab_name}_hdate")) {
                        if (!$this->getVal('titre_short'))
                                $this->set('titre_short', 'تاريخ ' . AfwStringHelper::arabicTaarif($my_tab_tu));
                        if (!$this->getVal('field_size'))
                ...
                if ($field_name == "${my_tab_name}_time") {
                        if (!$this->getVal('titre_short'))
                                $this->set('titre_short', 'وقت ' . AfwStringHelper::arabicTaarif($my_tab_tu));
                        if (!$this->getVal('field_size'))
                                $this->set('field_size', 8);
                        if (!$this->getVal('afield_type_id'))
                                $this->set('afield_type_id', 4);
                }

                if ($field_name == "${my_tab_name}_amount") {
                        if (!$this->getVal('titre_short'))
                                $this->set('titre_short', 'مبلغ ' . AfwStringHelper::arabicTaarif($my_tab_tu));
                        if (!$this->getVal('field_size'))
                                $this->set('field_size', 10);
                }
                ...
                if (AfwStringHelper::stringEndsWith($field_name, '_yn')) {
                        if (!$this->getVal('afield_type_id'))
                                $this->set('afield_type_id', AfwUmsPagHelper::$afield_type_yn);
                }
                ...
                if (($this->getVal('afield_type_id') == AfwUmsPagHelper::$afield_type_date) or
                        ...
                        if (!$this->getVal('titre_short') and (AfwStringHelper::stringEndsWith($field_name, '_start_hdate')))
                                $this->set('titre_short', 'تاريخ بداية xxxxxx بالهجري');
                        if (!$this->getVal('titre_short') and (AfwStringHelper::stringEndsWith($field_name, '_end_hdate')))
                                $this->set('titre_short', 'تاريخ نهاية xxxxxx بالهجري');
                        if (!$this->getVal('titre_short') and (AfwStringHelper::stringEndsWith($field_name, '_start_date')))
                                $this->set('titre_short', 'تاريخ بداية xxxxxx بالهجري');
                        if (!$this->getVal('titre_short') and (AfwStringHelper::stringEndsWith($field_name, '_end_date')))
                                $this->set('titre_short', 'تاريخ نهاية xxxxxx بالهجري');
                        if (!$this->getVal('titre_short') and (AfwStringHelper::stringEndsWith($field_name, '_start_gdat')))
                                $this->set('titre_short', 'تاريخ بداية xxxxxx بالميلادي');
                        if (!$this->getVal('titre_short') and (AfwStringHelper::stringEndsWith($field_name, '_end_gdat')))
                                $this->set('titre_short', 'تاريخ نهاية xxxxxx بالميلادي');
                ...
                if (AfwStringHelper::stringEndsWith($field_name, '_amount')) {
                        if (!$this->getVal('afield_type_id'))
                                $this->set('afield_type_id', 3);
                        if (!$this->getVal('titre_short'))
                                $this->set('titre_short', 'مبلغ ');
                }
                ...
                if (AfwStringHelper::stringEndsWith($field_name, '_num')) {
                        if (!$this->getVal('afield_type_id'))
                                $this->set('afield_type_id', 13);
                        if (!$this->getVal('titre_short'))
                                $this->set('titre_short', 'عدد ');
                }
                ...
                if (($field_name == "${my_tab_name}_date") or ($field_name == "${my_tab_name}_hdate")) {
                        if (!$this->getVal('titre_short'))
                                $this->set('titre_short', 'تاريخ ' . AfwStringHelper::arabicTaarif($my_tab_tu));
                        if (!$this->getVal('titre_short_en'))
                                $this->set('titre_short', $my_tab_tu_en . ' date');
                }
                ...
                if ($field_name == "${my_tab_name}_amount") {
                        if (!$this->getVal('titre_short'))
                                $this->set('titre_short', 'مبلغ ' . AfwStringHelper::arabicTaarif($my_tab_tu));
                        if (!$this->getVal('titre_short_en'))
                                $this->set('titre_short', $my_tab_tu_en . ' amount');
                }
                ...
                if (!$this->getVal('scenario_item_id')) {
                        ...
                        if ($pf)
                                $si_pf = $pf->getVal('scenario_item_id');
                        if ($nf)
                                $si_nf = $nf->getVal('scenario_item_id');
                ...
                if ($anstab) {
                        ...
                        if ((!$titre) or (!$titre_en) or ($titre == $titre_en)) {
                                if (($this->getVal('afield_type_id') == AfwUmsPagHelper::$afield_type_list)) {
                                        ...
                                        if ($titre_propose)
                                                $titre_propose = AfwStringHelper::arabicTaarif($titre_propose);
                ...
                if (($field_name) and
                        ...
                        if ($afield_similar) {
                                if (!trim($this->getVal('afield_type_id')))
                                        $this->set('afield_type_id', $afield_similar->getVal('afield_type_id'));
                                if (!trim($this->getVal('field_size')))
                                        $this->set('field_size', $afield_similar->getVal('field_size'));
                ...
                if ((AfwStringHelper::stringEndsWith($field_name, '_nb') or (AfwStringHelper::stringEndsWith($field_name, '_number')))) {
                        if (!$this->getVal('afield_type_id'))
                                $this->set('afield_type_id', 1);
                        if (!$this->getVal('titre_short'))
                                $this->set('titre_short', 'عدد ');
                        if (!$this->getVal('titre_short_en'))
                                $this->set('titre_short', 'number ');
                }
                ...
                if ($this->may('reel')) {
                        ...
                        if (!$this->getVal('afield_category_id')) {
                                ...
                                if ($spec_arr[0]) {
                                        ...
                                        if ($afcObj)
                                                $this->set('afield_category_id', $afcObj->getId());
                                }
                        }
                }
                if (!$this->getVal('field_width'))
                        $this->set('field_width', $this->getVal('field_size'));
        ...
        public function resetDefaultModes($commit = false, $my_tab = null)
        {
                ...
                if (($nb_mod_retrieve < 5) or (!$cur_retrieve) or ($cur_retrieve == 'W')) {
                        if (
                                ...
                                ($this->getVal('afield_type_id') == AfwUmsPagHelper::$afield_type_mediumint) or
                                ($this->getVal('afield_type_id') == AfwUmsPagHelper::$afield_type_smallint) or
                                ($this->getVal('afield_type_id') == AfwUmsPagHelper::$afield_type_bigint) or
        ...
        public static function props_to_foptions($thisObject, $atableObject, $afieldObject, $create_foption_if_not_exists = false)
        {
                ...
                foreach ($strcut as $prop => $prop_value) {
                        if ($prop and (!$prop_to_field_arr[$prop])) {
                                ...
                                if (!$foptionObj)
                                        $foptionObj = Foption::loadByMainIndex($prop, $create_foption_if_not_exists);
                                if ($foptionObj) {
                                        ...
                                        if (!$data_type_id) {
                                                ...
                                                if (is_numeric($prop_value))
                                                        $data_type_id = AfwUmsPagHelper::$afield_type_mediumint;
                                                if (is_float($prop_value))
                                                        $data_type_id = AfwUmsPagHelper::$afield_type_float;
                                                if (!$data_type_id)
                                                        $data_type_id = AfwUmsPagHelper::$afield_type_text;
                                        ...
                                        if ($af_type_id)
                                                $foptionObj->addRemoveInMfk('afield_type_mfk', array($af_type_id), array());
                                        if ($af_catg_id)
                                                $foptionObj->addRemoveInMfk('afield_category_mfk', array($af_catg_id), array());
                                        ...
                                        if ($prop_value or ($data_type_id == AfwUmsPagHelper::$afield_type_yn)) {
                                                if (is_string($prop_value))  // ($data_type_id==AfwUmsPagHelper::$afield_type_list)
                                                {
                                                        ...
                                                        if ($foc) {
                                                                if ($af_type_id)
                                                                        $foc->addRemoveInMfk('afield_type_mfk', array($af_type_id), array());
                                                                if ($af_catg_id)
                                                                        $foc->addRemoveInMfk('afield_category_mfk', array($af_catg_id), array());
        ...
        public static function to_afield_att($id_main_sh, $row)
        {
                ...
                if (!is_numeric($row['SIZE']))
                        $afield_att['field_width'] = 9999;
                else
                        $afield_att['field_width'] = ($row['SIZE'] > 0) ? $row['SIZE'] : 0;

                $afield_att['field_min_size'] = ($row['MIN-SIZE']) ? $row['MIN-SIZE'] : 0;

                if ($row['RELATION'] == 'ManyToOne')
                        $afield_att['entity_relation_type_id'] = self::$ENTITY_RELATION_TYPE_MANYTOONE;
                if ($row['RELATION'] == 'OneToMany')
                        $afield_att['entity_relation_type_id'] = self::$ENTITY_RELATION_TYPE_ONETOMANY;
                if ($row['RELATION'] == 'OneToOneB')
                        $afield_att['entity_relation_type_id'] = self::$ENTITY_RELATION_TYPE_ONETOONEBIDIRECTIONAL;
                if ($row['RELATION'] == 'OneToOneU')
                        $afield_att['entity_relation_type_id'] = self::$ENTITY_RELATION_TYPE_ONETOONEUNIDIRECTIONAL;

                if ($row['CHAR_TEMPLATE'])
                        $afield_att['char_group_men'] = self::char_template_to_mfk($row['CHAR_TEMPLATE']);
                 ...
                 */
                if ($row['READONLY'])
                        $afield_att['readonly'] = 'Y';
                else
                        $afield_att['readonly'] = 'N';
                ...
                if (!$afield_att['afield_type_id'])
                        $afield_att['afield_type_id'] = ($row['TYPE'] == 'FK') ? 5 : 0;
                if (!$afield_att['afield_type_id'])
                        $afield_att['afield_type_id'] = ($row['TYPE'] == 'MFK') ? 6 : 0;
        ...
        public function generePhpDesc($scis = null)
        {
                ...
                if (!$row['STEP']) {
                        if ($scis and count($scis) > 0) {
                                ...
                                foreach ($scis as $sci_id => $sci_item) {
                                        if (($field_order >= $sci_item->start) and ($field_order <= $sci_item->end))
                                                $step = $sci_item->valstep_num();
                                }
                ...
                if ($my_tab)
                        $table_abrev = $my_tab->getVal('abrev');
                else
                        $table_abrev = '';

                if ($this->getVal('shortname'))
                        $row['SHORTNAME'] = $this->getVal('shortname');
                ...
                if ((!$row['SHORTNAME']) and
                        ...
                        if ($count_words > 1) {
                                ...
                                if ($suggested_shortname) {
                                        if ($afield_type_id == AfwUmsPagHelper::$afield_type_list)
                                                $row['SHORTNAME'] = $suggested_shortname;
                                        if ($afield_type_id == AfwUmsPagHelper::$afield_type_enum)
                                                $row['SHORTNAME'] = $suggested_shortname;
                                        if ($afield_type_id == AfwUmsPagHelper::$afield_type_mlst)
                                                $row['SHORTNAME'] = $suggested_shortname . 's';
                                        if ($afield_type_id == AfwUmsPagHelper::$afield_type_menum)
                                                $row['SHORTNAME'] = $suggested_shortname . 's';
                                }
                        }
                }
                ...
                if ($this->getVal('field_width'))
                        $row['SIZE'] = $this->getVal('field_width');
                if ($this->getVal('field_size'))
                        $row['MAXLENGTH'] = $this->getVal('field_size');
                if ($this->getVal('field_min_size'))
                        $row['MIN-SIZE'] = $this->getVal('field_min_size');
                if ($this->getVal('char_group_men'))
                        $row['CHAR_TEMPLATE'] = '"' . self::mfk_to_char_template($this->getVal('char_group_men')) . '"';
                ...
                if ($this->getVal('field_format'))
                        $row['FORMAT'] = $this->getVal('field_format');
                if ($this->getVal('title_after'))
                        $row['TITLE_AFTER'] = $this->getVal('title_after');
                ...
                if ($afield_type_id == AfwUmsPagHelper::$afield_type_mtxt) {
                        ...
                        if ($this->getVal('answer_table_id')) {
                                ...
                                if (!$at_module)
                                        $at_module = $ansTabObj->het('id_module');
                                $row['ANSMODULE'] = "'" . $at_module->getVal('module_code') . "'";
                                if ($ansTabObj->hasManyData())
                ...
                if ($afield_type_id == AfwUmsPagHelper::$afield_type_date) {
                        ...
                        if (!$row['FORMAT'])
                                $row['FORMAT'] = "'HIJRI_UNIT'";  // CONVERT_NASRANI_SIMPLE
                }
                ...
                if (count($dependentList) > 0)
                        $row['DEPENDENT_OFME'] = AfwStringHelper::hzmArrayStringFormat(AfwWizardHelper::listToArray($dependentList, 'field_name'));
        ...
        public function getDisplay($lang = 'ar')
        {
                ...
                if (!$fn)
                        $fn = trim($this->getVal("titre$lang_suffix"));
                $id = $this->getId();
                if (!$fn)
                        $fn = "afield.$id";
        ...
        {
                if ($this->getVal('field_width') > 10)
                        return $this->getVal('field_width');
                else {
                        if ($this->getVal('afield_type_id') == 5)
                                return 25;
                        if ($this->getVal('afield_type_id') == 6)
                                return 25;
                        if ($this->getVal('afield_type_id') == 2)
                                return 25;
                        if ($this->getVal('afield_type_id') == 9)
                                return 25;
                }
        ...
        public function myJavaName()
        {
                ...
                if ($this->getVal('afield_type_id') == 5) {
                        ...
                        $anstab = $this->het('answer_table_id');
                        if (!$atab)
                                return "error : no table associated to $this_field_name($this_field_id) " . $this;
                        if (!$anstab)
                                return "error : no answer table for $this_field_name($this_field_id) " . $this;
        ...
        public function getGeneraltedSQL($dbms, $syntax_values, $fields_naming_uc, $alter_table = false)
        {
                ...
                $afield_type_id = $this->getVal('afield_type_id');
                if (!$afield_type_id)
                        return "getSQL::Error not defined field type for field `$field_name` of table `" . $my_tab->getVal('atable_name') . "`\n";
                $afield_type_obj = $this->get('afield_type_id');
                $field_type = $afield_type_obj->getVal("${dbms}_field_type");
                if ($field_size)
                        $field_type = str_replace('[SIZE]', $field_size, $field_type);
                $table_utf8 = ($my_tab->getVal('utf8') == 'Y');
                if ((!$table_utf8) and $field_utf8)
                        $field_type = str_replace('[UTF8]', $utf8_syntax, $field_type);
                else
                        $field_type = str_replace('[UTF8]', '', $field_type);
                ...
                if (!$field_mandatory) {
                        ...
                        if (!$this->sureIs('distinct_for_list')) {
                                ...
                                if ($this->sureIs('utf8'))
                                        $default_value = $this->getVal('default_value_utf8');
                                if (!$default_value)
                                        $default_value = $this->getVal('default_value');
                                if (!$default_value)
                                        $default_value = $this->getVal('default_value_utf8');
                                if (!$default_value)
                                        $default_value = $this->getStandardDefaultValue();
                ...
                if ($alter_table)
                        $alter_table_sql = "-- ALTER TABLE $mcode.$table_name add";
                else
                        $alter_table_sql = '';
                ...
                switch ($attribute) {
                        case 'fk_table_single':
                                ...
                                if ($lang_suffix == '_ar')
                                        $lang_suffix = '';
                                if ($fk_tab)
                                        return $fk_tab->getVal('singletitle' . $lang_suffix);
                        ...
                        case 'fk_table_plural':
                                ...
                                if ($lang_suffix == '_ar')
                                        $lang_suffix = '';
                                if ($fk_tab)
                                        return $fk_tab->getVal('pluraltitle' . $lang_suffix);
                                return 'no fk tab for field ' . $this . "($fk_tab_id)";
                                break;
                        ...
                        case 'fld_table_single':
                                ...
                                if ($lang_suffix == '_ar')
                                        $lang_suffix = '';
                                if ($mytable)
                                        return $mytable->getVal('singletitle' . $lang_suffix);
                                else
                                        return '';
                        ...
                        case 'fld_table_plural':
                                ...
                                if ($lang_suffix == '_ar')
                                        $lang_suffix = '';
                                if ($mytable)
                                        return $mytable->getVal('pluraltitle' . $lang_suffix);
                                else
                                        return '';
                        ...
                        case 'sql':
                                ...
                                if ($dbsystem)
                                        $dbms = strtolower($dbsystem->getVal('dbsystem_code'));
                                if ($dbms == 'mysql')
                                        $dbms = 'sql';

                                if ((!$dbms) or (!$dbsystem) or (!$dbengine))
                                        return "Please define db system and engine of module [$module] for the table [$mytable] of the field [$this]";
        ...
        protected function getOtherLinksArray($mode, $genereLog = false, $step = 'all')
        {
                ...
                if ($nf)
                        $next_field_id = $nf->getId();
                else
                        $next_field_id = 0;
                if ($pf)
                        $previous_field_id = $pf->getId();
                else
                        $previous_field_id = 0;
        ...
        public function getRelationFromName($name)
        {
                ...
                $anstab = $this->het('answer_table_id');
                if ($anstab)
                        $anstab_name = $anstab->getVal('atable_name');
                else
                        $anstab_name = '';
                ...
                if ($tab_name)
                        $str = str_replace('_' . $tab_name, '', $str);
                if ($anstab_name)
                        $str = str_replace('_' . $anstab_name, '', $str);
        ...
        public function getEnumTables()
        {
                ...
                if (!$answer_module_id)
                        $answer_module_id = $parent_module_id;
        ...
        protected function getSpecificDataErrors($lang = 'ar', $show_val = true, $step = 'all', $erroned_attribute = null, $stop_on_first_error = false, $start_step = null, $end_step = null)
        {
                ...
                if (($this->getVal('afield_type_id') == 5) and ($this->may('reel'))) {
                        if ((!$this->getVal('entity_relation_type_id')) or ($this->getVal('entity_relation_type_id') == 3))
                                $sp_errors['entity_relation_type_id'] = 'لا بد من تحديد نوع العلاقة مع جدول الاختيارات';
                }
        ...
        {
                if ($char_template == 'LOOKUP_CODE')
                        $char_template = 'ALPHABETIC,UNDERSCORE';
                if ($char_template == 'TEXT_AR')
                        $char_template = 'ARABIC-CHARS,SPACE';
                if ($char_template == 'TEXT_EN')
        ...
        public static function char_group_code_to_id($code)
        {
                ...
                if ($code == 'ALPHABETIC')
                        $id = 1;
                if ($code == 'ARABIC-CHARS')
                        $id = 2;
                if ($code == 'NUMERIC')
                        $id = 3;
                if ($code == 'MATH-SYMBOLS')
                        $id = 4;
                if ($code == 'BRACKETS')
                        $id = 5;
                if ($code == 'COMMAS')
        ...
        public function getRAMObjectData()
        {
                ...
                if (!$code)
                        throw new AfwRuntimeException('this field is without field name');
                ...
                if ($fieldCloned->insert()) {
                        if ($cloneOptions) {
                                foreach ($afieldOptionValueList as $afieldOptionValueItem) {
                                        if ($afieldOptionValueItem->cloneInField($fieldCloned->id))
                                                $nb_inserts++;
                                }
                        }
                }
        ...
        public function createShortcutFieldsForOneToOneObject($lang = 'ar')
        {
                ...
                if (!$shortname)
                        $err_arr[] = 'Error : please define shortname for this field';
                ...
                if (!$anstable)
                        $err_arr[] = 'Error : No Answer table for this field';

                $my_tab = $this->het('atable_id');

                if (!$my_tab)
                        $err_arr[] = 'Error : No owner table for this field';
                ...
                if ($anstable and $shortname) {
                        ...
                        $scStep = $this->hetStep();
                        if ($scStep)
                                $fromStep = $scStep->getVal('step_num');
        ...
        {
                if ($attribute == 'atable_name_ar')
                        return true;
                if ($attribute == 'atable_name_en')
                        return true;
                if ($attribute == 'atable_name')
                        return true;
                if ($attribute == 'sql_field_type')
                        return true;
                if ($attribute == 'mask')
                        return true;
                return false;
        ...
        public function myShortNameToAttributeName($attribute)
        {
                ...
                if ($attribute == 'table')
                        return 'atable_id';
                if ($attribute == 'ftype')
                        return 'afield_type_id';
                if ($attribute == 'title')
                        return 'titre_short_en';
                if ($attribute == 'step')
                        return 'scenario_item_id';
                if ($attribute == 'category')
                        return 'afield_category_id';
                if ($attribute == 'anstable')
                        return 'answer_table_id';
                if ($attribute == 'ertype')
                ...
                if ($attribute == 'myname')
                        return 'mode_name';
                if ($attribute == 'char_group')
                        return 'char_group_men';
                if ($attribute == 'php')
                        return 'php_att';
                return $attribute;
        }
        ...
        public static function addByCodes($object_code_arr, $object_name_en, $object_name_ar, $object_title_en, $object_title_ar, $update_if_exists = false, $command_code_option = '')
        {
                ...
                if (!$module_code or !$table_name or !$field_name)
                        throw new AfwRuntimeException("addByCodes : module and table name and field name are needed, given : module=$module_code and table=$table_name and field=$field_name , given array : " . var_export($object_code_arr, true));
                $objModule = Module::loadByMainIndex($module_code);
                ...
                if (!$objTable or (!$objTable->id))
                        throw new AfwRuntimeException("addByCodes : table $table_name not found in module  $module_code/$objModule_id");
                ...
                if (!$afObj)
                        $message = "Strange Error happened because Afield::loadByMainIndex($objTable_id, $field_name) failed !!";
                else {
                        if ((!$afObj->is_new) and (!$update_if_exists)) {
                                throw new AfwRuntimeException('This field already exists');
                        }
                        if ($field_type)
                                $afObj->set('afield_type_id', $field_type);
        ...
        {
                if (count($object_code_arr) != 3)
                        throw new AfwRuntimeException('reverseByCodes : 3 params are needed module and table and field name, given : ' . var_export($object_code_arr, true));
                $module_code = $object_code_arr[2];
                $table_name = $object_code_arr[1];
                $field_name = $object_code_arr[0];

                if (!$module_code or !$table_name or !$field_name)
                        throw new AfwRuntimeException("reverseByCodes : module and table field_name are needed, given : module=$module_code and table=$table_name and field=$field_name");
                $objModule = Module::loadByMainIndex($module_code);
                if (!$objModule or (!$objModule->id))
                        throw new AfwRuntimeException("reverseByCodes : module $module_code not found");
                $objModule_id = $objModule->id;
                $objTable = Atable::loadByMainIndex($objModule_id, $table_name);
                if (!$objTable or (!$objTable->id))
                        throw new AfwRuntimeException("reverseByCodes : table $table_name not found in module  $module_code/$objModule_id");
                $message = '';
                if ($doReverse) {
                        $tableClass = AfwStringHelper::tableToClass($table_name);
                        AfwAutoLoader::addModule($module_code);
                ...
                if ($objField)
                        $message .= " Field $field_name loaded successfully";
                else
                        $message .= "Error Field $field_name load failed";
