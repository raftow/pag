...
if ($goal_id > 0) {
    if (($action == "+t") or ($action == "-t")) {
        ...
        if (!$atable_name) {
            $command_line_result_arr[] = AfwUtils::hzm_format_command_line("error", "goal command need that you specify the table name see {help goal} examples");
            $nb_errors++;
        ...
        if (!$currmod) {
            $command_line_result_arr[] = AfwUtils::hzm_format_command_line("error", "goal command need that you select the current module");
            $nb_errors++;
            $command_finished = true;
            return;
        }
        ...
        if (!$objModule) {
            $command_line_result_arr[] = AfwUtils::hzm_format_command_line("error", "failed to load module with code = $currmod");
            $nb_errors++;
            $command_finished = true;
            return;
        } else {
            ...
            if ($objAtable and (!$objAtable->isEmpty())) {
                ...
                if ($full) {
                    ...
                    if (!$goalObj) {
                        $command_line_result_arr[] = AfwUtils::hzm_format_command_line("error", "failed to load goal with id = $goal_id");
                        $nb_errors++;
                        $command_finished = true;
                        return;
                    }
                ...
                if ($error) {
                    $command_line_result_arr[] = AfwUtils::hzm_format_command_line("error", "error while doing $what_done");
                    $nb_errors++;
                    $command_finished = true;
                    return;
                }

                $command_line_result_arr[] = AfwUtils::hzm_format_command_line("info", "while doing $what_done result was :");
                $command_line_result_arr[] = AfwUtils::hzm_format_command_line("success", $info);
                $command_line_result_arr[] = AfwUtils::hzm_format_command_line("info", $atable_translated . " has been managed by goal " . $goalObj->getDisplay($lang), $lang);
                $command_line_result_arr[] = AfwUtils::hzm_format_command_line("warning", "You can check now if your menus are changed", $lang);
                $command_finished = true;
                $command_done = true;
            } else {
                $command_line_result_arr[] = AfwUtils::hzm_format_command_line("error", "table $atable_name not found in module $currmod/$idMod");
                $nb_errors++;
                $command_finished = true;
                return;
            }
        ...
        $command_line_result_arr[] = AfwUtils::hzm_format_command_line("error", "action $action not found for goal command");
        $nb_errors++;
        $command_finished = true;
        return;
    }
